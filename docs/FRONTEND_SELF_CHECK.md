...
- **当前状态**：🟡（有改善）。已修 `lang=zh-CN`、`prefers-reduced-motion` 全局块、键盘快捷键作用域。本轮给排序 viz 加了**非颜色语义通道**：图例与柱区指针用形状区分（比较◇/交换⇄/完成✓），红绿色盲在灰度下也能分辨；`Legend` 组件支持可选 `symbol`，其他 viz 可复用。**遗留**：可视化整体 aria 标注仍薄；只做了排序类，图/树等其它 viz 的颜色语义未系统化。
- **问 AI 的话术**：
