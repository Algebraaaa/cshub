...
const DEFAULT_FLOW = {
  ...
  edges: [
    { from: 'S', to: 'A', cap: 10 }, { from: 'S', to: 'B', cap: 8 },
    { from: 'A', to: 'C', cap: 5 }, { from: 'A', to: 'D', cap: 7 },
    { from: 'B', to: 'D', cap: 10 }, { from: 'C', to: 'T', cap: 9 },
    { from: 'D', to: 'T', cap: 6 }, { from: 'C', to: 'D', cap: 3 },
  ],
  source: 'S',
  sink: 'T',
}

export default function DinicPlayground({ algoFn }) {
  return (
    <PlaygroundShell
      presets={[{ id: 'default', label: '默认网络（6 节点 8 边）' }]}
      computeSteps={() => algoFn(DEFAULT_FLOW)}
      renderViz={({ current }) => <DinicViz stepData={current} graph={DEFAULT_FLOW} />}
    />
  )
}

function DinicViz({ stepData, graph }) {
  if (!stepData) return null
  const { edges: edgeState, level, current, augmentPath, flow, phase, highlightEdges } = stepData
  const nodeMap = {}
    ...
    <div>
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
              ...
              return (
                <g key={i}>
                  ...
                  <polygon
                    ...
                    opacity={saturated && !hl ? 0.4 : 1}
                  />
                  <text x={mx + nx} y={my + ny} textAnchor="middle" fill="var(--text-secondary)" fontSize={10} fontWeight="600">
                    {e.flow}/{e.cap}
                  </text>
                </g>
              )
        ...
        <div style={{
          ...
          <div style={{ fontWeight: 700, color: 'var(--text-primary)', marginBottom: 8 }}>最大流</div>
          <div style={{ fontSize: 28, fontWeight: 800, color: 'var(--accent)', fontFamily: 'var(--font-mono)', textAlign: 'center', margin: '12px 0' }}>
            {flow ?? 0}
          </div>
          <div style={{ borderTop: '1px solid var(--border)', paddingTop: 8, marginTop: 8 }}>
            ...
            <div style={{ color: 'var(--text-secondary)', fontSize: 11 }}>
              {phase === 'init' && '初始化'}
              {phase === 'bfs' && 'BFS 构建层次图'}
              {phase === 'bfs_done' && 'BFS 完成'}
              {phase === 'dfs_start' && 'DFS 增广'}
              {phase === 'dfs' && 'DFS 推送流量'}
              {phase === 'augment' && '增广完成'}
              {phase === 'phase_done' && '阶段结束'}
              {phase === 'done' && '算法完成'}
            </div>
          </div>
          <div style={{ marginTop: 12, borderTop: '1px solid var(--border)', paddingTop: 8 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <LegendItem color="var(--blue)" label="当前边" />
            </div>
          </div>
        </div>
      </div>
