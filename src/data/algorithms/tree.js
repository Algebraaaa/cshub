...
int getBal(Node* n) { return n ? h(n->left) - h(n->right) : 0; }
...
def insert(root, val):
    ...
    bal = get_h(root.left) - get_h(root.right)
    
    if bal > 1 and val < root.left.val: return rotate_right(root)
    if bal < -1 and val > root.right.val: return rotate_left(root)
    ...
    return root`,
        ...
        treap: {
            ...
            pseudocode: `procedure insert(node, key, priority):
    if node is null: return newNode(key, priority)
    if key < node.key:
    ...
    int val, prio;
    Node *left, *right;
    Node(int v, int p) : val(v), prio(p), left(nullptr), right(nullptr) {}
};
...
Node* insert(Node* root, int val, int prio) {
    if (!root) return new Node(val, prio);
    if (val < root->val) {
    ...
    def __init__(self, val, prio):
        self.val, self.prio = val, prio
        self.left = self.right = None

def rotate_right(y):
    x = y.left; y.left = x.right; x.right = y; return x

def insert(root, val, prio):
    if not root: return Node(val, prio)
    if val < root.val:
        root.left = insert(root.left, val, prio)
        if root.left.prio < root.prio: root = rotate_right(root)
    else:
        root.right = insert(root.right, val, prio)
        if root.right.prio < root.prio: root = rotate_left(root)
    return root`,
            },
        },
    applications: [
