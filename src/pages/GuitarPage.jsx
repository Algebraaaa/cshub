...
import { GUITAR_CURRICULUM, GUITAR_TOTAL_LESSONS } from '../data/guitar/curriculum'
import { useCourseProgress } from '../features/music/hooks/useCourseProgress'

export default function GuitarPage() {
  const { progress, isCompleted } = useCourseProgress(GUITAR_CURRICULUM.id, GUITAR_TOTAL_LESSONS)
  const nextLesson = findNextLesson(GUITAR_CURRICULUM, isCompleted)
  const firstLesson = GUITAR_CURRICULUM.chapters[0].lessons[0]
    ...
    <div className="min-h-screen pb-24">
      ...
      <div className="relative overflow-hidden px-6 py-16 text-center">
        ...
        <div className="relative max-w-2xl mx-auto">
          ...
          <h1 className="text-4xl font-bold text-fg mb-3">{GUITAR_CURRICULUM.title}</h1>
          <p className="text-fg-muted text-base mb-6 max-w-lg mx-auto">
            从认识吉他构造到弹唱第一首完整民谣，附带和弦图、扫弦节奏和指弹示范。
          </p>
          ...
          <div className="flex gap-3 justify-center flex-wrap">
            <Link
              to={`/guitar/lesson/${nextLesson.id}`}
              className="btn-primary px-6 py-2.5 text-sm font-semibold"
            >
              {progress.count === 0 ? '开始学习' : '继续学习'} →
            </Link>
            <Link
              to={`/guitar/lesson/${firstLesson.id}`}
      ...
      <div className="max-w-3xl mx-auto px-6">
        <div className="flex flex-col gap-6">
          {GUITAR_CURRICULUM.chapters.map((chapter, ci) => {
                      ...
                      <li key={lesson.id}>
                        ...
                        >
                          <span
                            ...
                            style={{
                              background: done ? '#10b98122' : 'var(--surface)',
                              color: done ? '#10b981' : 'var(--text-tertiary)',
                              border: `1px solid ${done ? '#10b98144' : 'var(--border)'}`,
                            }}
                          >
                            {done ? '✓' : `${ci + 1}.${li + 1}`}
                          </span>
